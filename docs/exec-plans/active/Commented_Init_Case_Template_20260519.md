...
- Implementation Notes: 普通模式提示 `input_types`、`output_type`、`inplace_write`、`inplace_index`、`unordered_output`，并提供单链表、双向链表、循环链表和二叉树的配置示例；operations 模式提示三数组对齐和构造函数规则。
...
- Expected Tests: 本地 raw、linked_list、binary_tree、matrix 模板仍可解析，且包含高级类型注释示例。
...
- Expected Tests: 远程普通题和 operations 题模板仍可解析，普通题模板包含高级类型注释示例。
