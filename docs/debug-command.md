...
Leet-Chaser 需要新增 `debug` 命令，用于针对一个单独的本地 case 调试 `solution.py` 里的入口函数。命令形式为：
...
- `<problem-dir>/solution.py` 提供 LeetCode 风格的 `Solution` 类。
...
- `debug_problem(problem_dir, case_path=None, traces=())`：读取 debug TOML，加载 solution，使用 `snoop.snoop(watch=traces)` 包装入口方法并执行单个 case。
